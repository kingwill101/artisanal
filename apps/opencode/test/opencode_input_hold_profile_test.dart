...
import 'package:opencode/opencode.dart' as opencode;

void main() {
  test('OpenCode held-key profile detects pause growth', () async {
    ...
    try {
      ...
      final firstAvgMs =
          firstThird.fold<int>(0, (s, v) => s + v) / firstThird.length / 1000.0;
      final lastAvgMs =
  ...
  test('OpenCode held-key cadence profile detects burst spikes', () async {
    final tester = WidgetTester(screenWidth: 120, screenHeight: 40);
    try {
      await tester.pumpWidget(opencode.OpenCodeApp());
      ...
      print('Held-key cadence p95    : ${(p95Us / 1000).toStringAsFixed(2)}ms');
      print(
  ...
  test('OpenCode paste profile measures large paste latency', () async {
    final tester = WidgetTester(screenWidth: 120, screenHeight: 40);
    try {
      await tester.pumpWidget(opencode.OpenCodeApp());
