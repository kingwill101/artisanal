...
Future<w.Theme> loadOpenCodeThemeAtLaunch({
  ...
  if (path == null) {
    final source =
        openCodeBuiltinAssets['themes/${_normalizeThemeName(themeName)}'];
    if (source != null) {
      try {
        return _activateTheme(
          OpenCodeThemeDocument.fromJsonString(source),
          themeName,
        );
      } catch (error) {
        stderr.writeln(
          '[opencode] Failed to load built-in theme "$themeName": $error',
        );
        return _activeTheme;
      }
    }
  ...
  try {
    ...
    return _activateTheme(document, themeName);
  } catch (error) {
    stderr.writeln(
...
w.Theme _activateTheme(OpenCodeThemeDocument document, String themeName) {
...
Future<List<String>> discoverOpenCodeThemeNames() async {
  ...
  names.addAll(
    openCodeBuiltinAssets.keys
        .where((key) => key.startsWith('themes/'))
        .map((key) => key.substring('themes/'.length)),
  );
...
Iterable<String> _candidateThemeDirectories() sync* {
  ...
  yield '$cwd/apps/opencode/themes';
  yield '$cwd/themes';
  yield '$cwd/themes';
