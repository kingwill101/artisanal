...
class ZoneScanner {
  ...
  int _printableWidth(String s) {
    // Use the stringWidth function which handles ANSI sequences
    return stringWidth(_stripAnsi(s));
  }

  /// Strips ANSI escape sequences from a string.
  String _stripAnsi(String s) {
    // Simple ANSI stripper - removes ESC[...m and ESC[...z sequences
    final buffer = StringBuffer();
    var i = 0;
    while (i < s.length) {
      if (i < s.length - 1 &&
          s.codeUnitAt(i) == 0x1B &&
          s.codeUnitAt(i + 1) == 0x5B) {
        // Skip ESC[
        i += 2;
