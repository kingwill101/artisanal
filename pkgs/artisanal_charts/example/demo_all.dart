...
// Exit: Ctrl+C / q
//
// Run: dart run example/demo_all.dart
import 'dart:math' as math;

import 'package:artisanal/style.dart' hide Padding, Align, Border;
...
ChartPaintCallback _fullChart(int index, math.Random rng) {
  return (fb, w, h) {
    switch (index) {
      case 0:
        renderLineChart(
...
ChartPaintCallback _gridChart(int index, math.Random rng) {
  return (fb, w, h) {
    switch (index) {
      case 0:
        renderLineChart(
          ...
          rowChildren.add(
            SizedBox(
              ...
              child: ChartView(
                width: gridCellW,
                height: gridCellH,
                paintCallback: _gridChart(i, _rng),
              ),
            ),
          );
          if (col < gridCols - 1) {
        ...
        children: [
          Expanded(
            child: ChartView(
              width: fullW,
              height: fullH,
              paintCallback: _fullChart(_currentIdx, _rng),
            ),
          ),
          footer,
        ],
