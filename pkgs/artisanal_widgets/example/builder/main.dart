// Builder Widget Example
//
// Demonstrates using Builder to create widgets with inline callbacks
// that have access to BuildContext.
//
// Run with: dart run example/builder/main.dart

import 'package:artisanal/style.dart' hide Padding, Align;
import 'package:artisanal/tui.dart' as tui;
import 'package:artisanal_widgets/artisanal_widgets.dart' as w;

void main() async {
  final app = tui.WidgetApp(BuilderExample());
  await tui.runProgram(
    app,
    options: const tui.ProgramOptions(
      altScreen: true,
      mouseMode: tui.MouseMode.allMotion,
    ),
  );
}

class BuilderExample extends w.StatefulWidget {
  BuilderExample({super.key});

  @override
  w.State createState() => _BuilderExampleState();
}

class _BuilderExampleState extends w.State<BuilderExample> {
  final w.WidgetScrollController _scrollController = w.WidgetScrollController();

  @override
  w.Widget build(w.BuildContext context) {
    final theme = widget.theme;
    final label = theme.labelSmall.copy()..foreground(theme.onBackground);

    return w.Container(
      padding: const w.EdgeInsets.all(1),
      color: theme.background,
      child: w.Container(
        child: w.Scrollbar(
          controller: _scrollController,
          thickness: 1,
          gap: 1,
          enableHover: true,
          trackChar: ' ',
          thumbChar: ' ',
          trackUsesBackground: true,
          thumbUsesBackground: true,
          trackGradient: w.ScrollbarGradient.background(
            start: w.hasDarkBackground
                ? const BasicColor('#2f363d')
                : const BasicColor('#e3e7eb'),
            end: w.hasDarkBackground
                ? const BasicColor('#1f252a')
                : const BasicColor('#d3d9e0'),
          ),
          thumbGradient: w.ScrollbarGradient.background(
            start: w.hasDarkBackground
                ? const BasicColor('#3fb2ff')
                : const BasicColor('#2f7df6'),
            end: w.hasDarkBackground
                ? const BasicColor('#7c5cff')
                : const BasicColor('#6e55f5'),
          ),
          hoverThumbGradient: w.ScrollbarGradient.background(
            start: w.hasDarkBackground
                ? const BasicColor('#79ddff')
                : const BasicColor('#4f93ff'),
            end: w.hasDarkBackground
                ? const BasicColor('#b18bff')
                : const BasicColor('#836bff'),
          ),
          hoverThumbChar: ' ',
          child: w.ScrollView(
            controller: _scrollController,
            handleKeys: true,
            child: w.Column(
              gap: 1,
              children: [
                w.Text('Builder Widget', style: theme.titleLarge),
                w.Text('q: quit', style: label),
                w.Divider(width: 50),

                // Basic Builder — inline widget creation with context access
                w.Text(
                  'Basic Builder with theme access:',
                  style: theme.titleMedium,
                ),
                w.Builder(
                  builder: (ctx) {
                    final t = w.ThemeScope.of(ctx);
                    return w.Container(
                      padding: const w.EdgeInsets.symmetric(
                        horizontal: 2,
                        vertical: 1,
                      ),
                      decoration: w.BoxDecoration(border: Border.rounded),
                      child: w.Text(
                        'Built dynamically with Builder!',
                        style: Style().foreground(t.primary),
                      ),
                    );
                  },
                ),

                w.Divider(width: 50),

                // Builder returning a complex tree
                w.Text('Builder with complex tree:', style: theme.titleMedium),
                w.Builder(
                  builder: (ctx) {
                    final t = w.ThemeScope.of(ctx);
                    return w.Row(
                      gap: 2,
                      children: [
                        w.Container(
                          padding: const w.EdgeInsets.all(1),
                          decoration: w.BoxDecoration(border: Border.rounded),
                          child: w.Text(
                            'Box A',
                            style: Style().foreground(t.primary),
                          ),
                        ),
                        w.Container(
                          padding: const w.EdgeInsets.all(1),
                          decoration: w.BoxDecoration(border: Border.rounded),
                          child: w.Text(
                            'Box B',
                            style: Style().foreground(t.success),
                          ),
                        ),
                        w.Container(
                          padding: const w.EdgeInsets.all(1),
                          decoration: w.BoxDecoration(border: Border.rounded),
                          child: w.Text(
                            'Box C',
                            style: Style().foreground(t.warning),
                          ),
                        ),
                      ],
                    );
                  },
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }

  @override
  tui.Cmd? handleUpdate(tui.Msg msg) {
    if (msg is tui.KeyMsg && msg.key.char == 'q') {
      return tui.Cmd.quit();
    }
    return null;
  }
}
