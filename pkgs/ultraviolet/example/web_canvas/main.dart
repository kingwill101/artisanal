...
void main() {
  ...
  const cols = 80;
  const rows = 24;
  final renderer = CanvasTerminalRenderer(
  ...
  void write(int x, int y, String text, {UvStyle? style}) {
    for (var i = 0; i < text.length; i++) {
      buf.setCell(
        x + i,
        y,
        Cell(content: text[i], style: style ?? const UvStyle()),
      );
    }
  }
  ...
  for (var i = 0; i < 8; i++) {
    write(2 + i * 4, 5, '███ ', style: UvStyle(bg: UvColor.basic16(i)));
    write(
      2 + i * 4,
      6,
      '███ ',
  ...
  write(
    6,
    23,
    'Canvas renderer · ultraviolet · dart compile wasm',
    style: const UvStyle(fg: UvColor.indexed256(8)),
  );
