...
List<RemotePluginServiceDescriptor> _bindingsToServiceDescriptors(
  Map<String, Map<String, _RemotePluginGenericServiceBinding>> handlers,
) {
  final descriptors = <RemotePluginServiceDescriptor>[];
  final sortedServices = handlers.keys.toList()..sort();
  for (final service in sortedServices) {
    final methods = handlers[service]!;
...
final class RemotePluginGenericServiceCatalog {
  ...
  List<RemotePluginServiceDescriptor> get serviceDescriptors =>
      _bindingsToServiceDescriptors(_handlers);

  RemotePluginGenericHostService bind(RemotePluginHostConnection connection) {
    final service = RemotePluginGenericHostService.bind(connection);
    final sortedServices = _handlers.keys.toList()..sort();
    for (final serviceName in sortedServices) {
      final methods = _handlers[serviceName]!;
      final sortedMethods = methods.keys.toList()..sort();
      for (final methodName in sortedMethods) {
        final binding = methods[methodName]!;
        service.register(
          serviceName,
          methodName,
          binding.handler,
          description: binding.description,
          paramsSchema: binding.paramsSchema,
...
final class RemotePluginGenericHostService {
  ...
  List<RemotePluginServiceDescriptor> get serviceDescriptors {
    return _bindingsToServiceDescriptors(_handlers);
  }
