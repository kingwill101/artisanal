...
void _handleSpecialKey(web.KeyboardEvent event, WebTerminalBackend backend) {
  switch (event.key) {
    case 'Enter':
      backend.addInput([0x0D]);
    case 'Backspace':
      backend.addInput([0x7F]);
    case 'Escape':
      backend.addInput([0x1B]);
    case 'Tab':
      backend.addInput([0x09]);
    case 'ArrowUp':
      backend.addInput([0x1B, 0x5B, 0x41]);
    case 'ArrowDown':
      backend.addInput([0x1B, 0x5B, 0x42]);
    case 'ArrowRight':
      backend.addInput([0x1B, 0x5B, 0x43]);
    case 'ArrowLeft':
      backend.addInput([0x1B, 0x5B, 0x44]);
    case 'Delete':
      backend.addInput([0x1B, 0x5B, 0x33, 0x7E]);
    case 'Home':
      backend.addInput([0x1B, 0x5B, 0x48]);
    case 'End':
      backend.addInput([0x1B, 0x5B, 0x46]);
    case 'PageUp':
      backend.addInput([0x1B, 0x5B, 0x35, 0x7E]);
    case 'PageDown':
      backend.addInput([0x1B, 0x5B, 0x36, 0x7E]);
    case 'Control':
    case 'Shift':
    case 'Alt':
    case 'Meta':
