...
void main() {
  group('BrowserTerminalHostServer', () {
    test(
      'defaultPageHtml wires the websocket path and browser terminal helpers',
      () {
        ...
        expect(
          html,
          contains('const toolbarNode = document.querySelector(\'.toolbar\');'),
        );
        ...
        expect(
          html,
          contains(r"const resetForegroundSequence = '\x1b]110\x07';"),
        );
        expect(
          html,
          contains(r"const resetBackgroundSequence = '\x1b]111\x07';"),
        );
        ...
        expect(
          html,
          contains(
            r"const reportedSecondaryDeviceAttributes = '\x1b[>0;0;0c';",
          ),
        );
        expect(
          html,
          contains(
            r"const reportedTertiaryDeviceAttributes = '\x1bP!|787465726d2e6a73\x1b\\';",
          ),
        );
        ...
        expect(
          html,
          contains('function termcapResponsePayload(requestPayload)'),
        );
        ...
        expect(
          html,
          contains('const resizeObserver = new ResizeObserver(() => {'),
        );
        ...
        expect(
          html,
          contains(r"data: '\x1bP1+r' + responsePayload + '\x1b\\'"),
        );
        expect(html, contains('data: reportedKittyKeyboard'));
        expect(
          html,
          contains("encodeHexBytes('TN') + '=' + encodeHexBytes('xterm.js')"),
        );
        expect(html, contains('navigator.clipboard.readText()'));
        expect(
          html,
          contains('navigator.clipboard.writeText(text).catch(() => {});'),
        );
        ...
        expect(
          html,
          contains("activeDefaultPalette[paletteReset.index] ?? '#000000'"),
        );
        expect(html, contains('setBrowserTitle(title.title);'));
        expect(
          html,
          contains('currentForeground = activeDefaultTheme.foreground;'),
        );
        expect(
          html,
          contains('currentBackground = activeDefaultTheme.background;'),
        );
        ...
        expect(
          html,
          contains('const reply = oscColorReply(10, currentForeground);'),
        );
        expect(
          html,
          contains('const reply = oscColorReply(11, currentBackground);'),
        );
        expect(
          html,
          contains('const reply = oscColorReply(12, currentCursor);'),
        );
        expect(
          html,
          contains('currentSelectionBackground = theme.selectionBackground;'),
        );
        expect(
          html,
          contains('currentForeground = activeDefaultTheme.foreground;'),
        ...
        expect(
          html,
          contains(
            r"`\x1b[?${modeReport.mode};${modeReportValue(modeReport.mode)}$y`",
          ),
        );
        ...
        expect(html, contains(r"data: `\x1b[6;${cell.height};${cell.width}t`"));
        expect(
          html,
          contains("window.addEventListener('resize', scheduleResize);"),
        );
    ...
    test('bind serves the page and a 404 for unknown routes', () async {
      ...
      final pageBody = await pageResponse
    ...
    test(
      'failed websocket upgrades return bad request and keep the host alive',
      () async {
        final server = await BrowserTerminalHostServer.bind(
          port: 0,
    ...
    test(
      'synchronous session handler errors still clean up websockets',
      () async {
        var accepted = 0;
        final server = await BrowserTerminalHostServer.bind(
          port: 0,
