...
void main() {
  group('TextDocumentEditOps', () {
    test('replaceDocumentRange mutates the document and reports change metadata', () {
    ...
    test('insertIntoDocument inserts at the cursor without flattening first', () {
      final document = TextDocument(text: 'ab\ncd');

      final result = insertIntoDocument(
        document,
        3,
        const ['X', 'Y'],
      );
      ...
      expect(result.change.startPosition, const TextPosition(line: 1, column: 0));
      expect(result.change.newEndPosition, const TextPosition(line: 1, column: 2));
    });
